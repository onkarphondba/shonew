...
public class CnvInvoiceInfoStoredProcedureTasklet implements Tasklet, StepExecutionListener {
	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
		...
		Map<String, Object> result = batchItemService.callCnvInvoiceInfoProcedure();
		
		if((Integer)result.get(BatchConstants.ERROR_CODE) != 0){
			throw ExceptionHandler.createException(BatchConstants.CNV_STOREDPROCEDURE_ERROR, BatchConstants.POS_CNVINVOICEINFO_FRIENDLY_NAME, (String)result.get(BatchConstants.ERROR_MESSAGE));
		}
