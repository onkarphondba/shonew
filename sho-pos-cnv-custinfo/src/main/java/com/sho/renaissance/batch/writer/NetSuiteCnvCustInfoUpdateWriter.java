 ...
 * @author parthaka
 *
 */public class NetSuiteCnvCustInfoUpdateWriter extends JdbcBatchItemWriter implements StepExecutionListener {

	private static Log logger = LogFactory.getLog(NetSuiteCnvCustInfoUpdateWriter.class);
	...
	public void write(List items) throws Exception {
		List<CnvCustInfoEnriched> cnvCustInfoEnriched = (List<CnvCustInfoEnriched>) items;
		_port = nsUtils.getPort();
		List<CnvCustInfoEnriched> updatedData = createStoreInvFeed(cnvCustInfoEnriched);
	 ...
	 * @return
	 */
	private Customer getCustomRecord(CnvCustInfoEnriched temp) {
		Customer customer = new Customer();
		customer.setEmail(temp.getEmail());
		customer.setIsPerson((temp.getCustomerType().equals("pos")?true:false));
	    ...
	    RecordRef recRefCat = new RecordRef();
	    recRefCat.setInternalId("2");
	    customer.setCategory(recRefCat);
	   
	    RecordRef recRefSub = new RecordRef();
	    recRefSub.setInternalId("1");
	    customer.setSubsidiary(recRefSub);
	    
	    CustomerAddressbookList addressBookList = new CustomerAddressbookList();
	    CustomerAddressbook[] customerAddressBook = new CustomerAddressbook[1]; 
	    customerAddressBook[0].setDefaultShipping((temp.getDefaultShippingFlag().equals("Y")?true:false));
	    ...
	    customerAddressBook[1].setDefaultBilling(temp.getDefaultBillingFlag().equals("Y")?true:false);
	    ...
	    StringCustomFieldRef[] custFieldRef = new StringCustomFieldRef[0];
	 ...
	 * @throws Exception
	 */
	public List<CnvCustInfoEnriched> createStoreInvFeed(List<CnvCustInfoEnriched> cnvCustInfoEnriched) throws JsonGenerationException, JsonMappingException, IOException, DatatypeConfigurationException {


		Customer[] enrichCustomerList = new Customer[cnvCustInfoEnriched.size()];
		int i = cnvCustInfoEnriched.size() - 1;
		for (CnvCustInfoEnriched temp : cnvCustInfoEnriched) {
		...
		if (logger.isDebugEnabled()) {
			...
			writeRes = _port.updateList(enrichCustomerList);
		...
		for (CnvCustInfoEnriched temp : cnvCustInfoEnriched) {
			...
			if (writeRes.getWriteResponse(count).getStatus().isIsSuccess()) {
				...
				exception = ExceptionHandler.createException("sho.common.error.technical.sql.update.send_to_netsuite_failed", BatchConstants.POS_CNV_CUSTINFO_ENTITY,
						temp.getShcCustomerIndentifier(), temp.getSubsidiaryInternalId());
